CREATE OR REPLACE PACKAGE BODY deck_pkg IS
  ...
  FUNCTION get_card_value
    ( v_face_value IN OUT VARCHAR2,   -- The face value of the card
      v_hand_value IN NUMBER )  -- The hand value before this card is added
  ...
  FUNCTION deal_game( p_deck_pos IN NUMBER, p_num_players IN NUMBER )
    ...
    v_p1_hand VARCHAR2(255) := '';
    v_p2_hand VARCHAR2(255) := '';
    v_p3_hand VARCHAR2(255) := '';
    v_p4_hand VARCHAR2(255) := '';
  
    v_round_result VARCHAR2(255) := '';
  
    v_hands_to_deal NUMBER := game_pkg.v_player_count + 1;
    ...
    v_cur_player := 'Dealer';
      ...
      CASE i
      WHEN 1 THEN 
      v_loop_value := v_p1_v_hand_value;
      UPDATE ScoreTracker
        SET playerScore = v_loop_value
      WHERE playerNum = i;
    WHEN 2 AND p_player_num < hands_to_deal THEN 
      v_loop_value := v_p2_v_hand_value;
      UPDATE ScoreTracker
        SET playerScore = v_loop_value
      WHERE playerNum = i;
      WHEN 3 AND p_player_num < hands_to_deal THEN 
        v_loop_value := v_p3_v_hand_value;
        UPDATE ScoreTracker
        SET playerScore = v_loop_value
        WHERE playerNum = i;
      WHEN 4 AND p_player_num < hands_to_deal THEN 
        v_loop_value := v_p4_v_hand_value;
        UPDATE ScoreTracker
        SET playerScore = v_loop_value
        WHERE playerNum = i;
      ELSE 
        v_loop_value := v_dealer_v_hand_value;
        UPDATE ScoreTracker
        SET playerScore = v_loop_value
        WHERE playerNum = 5;
      END CASE;
